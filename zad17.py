...
parser = argparse.ArgumentParser(
    description="Uniwersalny agent przeszukujący strony (multi-engine)"
)
parser.add_argument(
    "--engine",
...
if not all([REPORT_URL, CENTRALA_API_KEY, QUESTIONS_URL, SOFTO_URL]):
    print(
        "❌ Brak wymaganych zmiennych: REPORT_URL, CENTRALA_API_KEY, SOFTO_QUESTIONS_URL, SOFTO_URL",
        file=sys.stderr,
    )
    sys.exit(1)
...
def select_best_link(
    links: List[Dict[str, str]], question: str, visited_urls: Set[str]
) -> Optional[str]:
...
def search_answers_node(state: PipelineState) -> PipelineState:
    ...
    for q_id, search_state in search_states.items():
        ...
        while search_state["search_depth"] < max_depth and not search_state["found"]:
            ...
            next_url = select_best_link(
                links, search_state["question_text"], search_state["visited_urls"]
            )
...
def send_answers_node(state: PipelineState) -> PipelineState:
    ...
    # Przygotuj payload
    payload = {"task": "softo", "apikey": CENTRALA_API_KEY, "answer": answers}
